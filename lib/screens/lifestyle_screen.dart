...
class _LifestyleScreenState extends State<LifestyleScreen> {
  ...
  Future<void> _edit() async {
    ...
    final alcoholController = TextEditingController(
      text: current?.alcoholFrequency ?? '',
    );
    final foodQualityController = TextEditingController(
      text: current?.foodQuality ?? '',
    );
    final milkTypeController = TextEditingController(
      text: current?.milkType ?? '',
    );
    final waterTypeController = TextEditingController(
      text: current?.waterType ?? '',
    );
    ...
    final saved = await showDialog<bool>(
      ...
      builder: (dialogContext) {
        return MedicalSaveDialog(
          ...
          validate: () => socioeconomicClass.trim().isEmpty
              ? 'Socioeconomic class is required.'
              : null,
          onSave: () async {
            ...
            return Column(
              ...
              children: [
                DropdownButtonFormField<String>(
                  ...
                  onChanged: saving
                      ? null
                      : (v) => socioeconomicClass = v ?? 'unknown',
                ),
                ...
                DropdownButtonFormField<bool?>(
                  ...
                  onChanged: saving ? null : (v) => livesAlone = v,
                ),
                const SizedBox(height: 12),
                DropdownButtonFormField<bool?>(
                  ...
                  onChanged: saving ? null : (v) => hasCaregiver = v,
                ),
                const SizedBox(height: 12),
                DropdownButtonFormField<bool?>(
                  initialValue: stairsInHome,
                  decoration: const InputDecoration(
                    labelText: 'Stairs in home',
                  ),
                  items: const [
                    DropdownMenuItem(value: null, child: Text('Unknown')),
                    DropdownMenuItem(value: true, child: Text('Yes')),
                  ...
                  onChanged: saving ? null : (v) => stairsInHome = v,
                ),
                const SizedBox(height: 12),
                TextField(
                ...
                DropdownButtonFormField<bool?>(
                  ...
                  onChanged: saving ? null : (v) => smoking = v,
                ),
                const SizedBox(height: 12),
                TextField(
                ...
                DropdownButtonFormField<bool?>(
                  ...
                  onChanged: saving ? null : (v) => drugs = v,
                ),
                const SizedBox(height: 12),
                TextField(
                ...
                DropdownButtonFormField<bool?>(
                  ...
                  onChanged: saving ? null : (v) => chicha = v,
                ),
                const SizedBox(height: 12),
                TextField(
                      ...
                      item == null
                          ...
                          : [
                              ...
                              if ((item.foodQuality ?? '').isNotEmpty)
                                'Food quality: ${item.foodQuality}',
                              if ((item.milkType ?? '').isNotEmpty)
                                'Milk type: ${item.milkType}',
                              if ((item.waterType ?? '').isNotEmpty)
                                'Water type: ${item.waterType}',
                            ].join('\n'),
                    ),
                  ),
                ),
