...
class _CaregiverProfileScreenState extends State<CaregiverProfileScreen> {
  ...
  Future<String?> _saveAddress() async {
    // The caregiver profile service only saves the profile row.
    // Address stays as a separate table row so the schema remains consistent
    // with the DB foreign key from caregiver_profiles.address_id -> addresses.id.
  ...
  Future<void> _delete() async {
    ...
    try {
      ...
      // Deleting the profile should not automatically delete the address row,
      // because the address can be reused by other linked records if needed.
      _profileId = null;
      _addressId = null;
